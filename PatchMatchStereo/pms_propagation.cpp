...
void PMSPropagation::ViewPropagation(const sint32& x, const sint32& y) const
{
	...
	// 把p的视差平面转换坐标系后赋给q，计算新代价，如果新代价小于q的当前代价，就把该视差平面赋给q，并更新q的代价值。
	const sint32 q = y * width_ + xr;
	auto& plane_q = plane_right_[q]; //右视图上同名点 q 所在的平面
	auto& cost_q = cost_right_[q]; //右视图 q原来的 cost

	//左视图 p 和 右视图 q 应该是同一个点。
	//将p 的平面转化到右边，作为 q 所在的平面
...
void PMSPropagation::PlaneRefine(const sint32& x, const sint32& y) const
{
	...
	std::mt19937 gen(rd());
	// float32 rand_d = (*rand_disp_)(gen);

	// float32 rand_n= (*rand_norm_)(gen);
	...
	while (disp_update > stop_thres) {
		...
		float32 disp_rd = (*rand_disp_)(gen) * disp_update;
		...
		PVector3f norm_rd;
		if (!option_.is_fource_fpw) {
			norm_rd.x = (*rand_norm_)(gen) * norm_update;
			norm_rd.y = (*rand_norm_)(gen) * norm_update;
			float32 z = (*rand_norm_)(gen) * norm_update;
			while (z == 0.0f) {
				z = (*rand_norm_)(gen) * norm_update;
			}
			norm_rd.z = z;
		}
		else {
